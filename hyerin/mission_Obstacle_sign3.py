...
SERVO_CENTER = 570
SERVO_LEFT_MAX = 680
SERVO_RIGHT_MAX = 440   # ✅ 단독 코드 값 유지

ROI_HEIGHT_RATIO = 0.6
ROI_X_LEFT_RATIO = 0.3125
ROI_X_RIGHT_RATIO = 0.6875

STEERING_OFFSET = -2    # ✅ 단독 코드 값 유지
L_THRESHOLD = 230       # ✅ 단독 코드 값 유지

# ==========================================================
# [2] 라이다/장애물 회피 (기존 통합 구조)
# ==========================================================
LIDAR_PORT = 'COM3'

OBSTACLE_START_DIST = 1300   # mm (원 코드 기준)
SHIFT_GAIN = 2.0             # px per (mm 부족분)
OBSTACLE_CLEAR_TIME = 1.5

# 가까우면 "멈추는" 기능이 있냐고 했지?
# -> 아래 STOP_DIST를 추가하면 "가까우면 정지"가 가능함 (기본은 비활성)
ENABLE_EMERGENCY_STOP = True
STOP_DIST = 350  # mm: 이 거리보다 가까우면 final_speed=0 (원하면 튜닝)
...
# ✅ 오토로 쓸 거니까 노출 관련 set(15, -6) 같은 건 하지 않음
...
def map_servo(angle):
    # angle: -45 ~ 45 -> SERVO_LEFT_MAX ~ SERVO_RIGHT_MAX
    angle = max(-45.0, min(45.0, float(angle)))
    return int((angle - (-45.0)) * (SERVO_RIGHT_MAX - SERVO_LEFT_MAX) / (45.0 - (-45.0)) + SERVO_LEFT_MAX)
...
def detect_traffic_lr_robust(frame_bgr):
    ...
    if red_left_ratio > COLOR_TH and green_right_ratio < COLOR_TH:
        return "LEFT", {"box": (x1, y1, x2, y2), "mode": "HSV_COLOR", "red_left": red_left_ratio, "green_right": green_right_ratio}
    if green_right_ratio > COLOR_TH and red_left_ratio < COLOR_TH:
        return "RIGHT", {"box": (x1, y1, x2, y2), "mode": "HSV_COLOR", "red_left": red_left_ratio, "green_right": green_right_ratio}
    ...
    return state, {"box": (x1, y1, x2, y2), "mode": "BRIGHT_BLOB", "thr": thr, "cx": cx, "area": area,
                   "red_left": red_left_ratio, "green_right": green_right_ratio}
...
def calculate_avoid_angle(image, left_line, right_line, obstacle_dist, last_angle, direction):
    ...
    # ✅ 베이스 타겟은 "단독 라인트레이싱"의 타겟 규칙을 그대로 사용
    if left_line is not None and right_line is not None:
    ...
    # ✅ 오프셋 적용(단독 코드 반영)
    base_target = base_target + STEERING_OFFSET
    base_target = max(0, min(width, base_target))
    ...
    final_target = max(0, min(width, final_target))
...
try:
    ...
    for scan in scan_iter:
        ...
        # (4) 차선 마스크 (✅ 고정 L_THRESHOLD + 오토 노출 전제)
        blurred = cv2.medianBlur(frame_lane, 5)
        ...
        # (6-1) "가까우면 정지" (옵션)
        # ==========================================================
        if ENABLE_EMERGENCY_STOP and (raw_dist <= STOP_DIST) and (not is_crosswalk_stop):
            final_speed = 0
            status_msg = f"EMERGENCY STOP ({raw_dist:.0f}mm)"
        ...
        # 추가 감속(정지 중 제외) - 기존 로직 유지
        if (raw_dist < OBSTACLE_START_DIST) and (not is_crosswalk_stop) and (not (ENABLE_EMERGENCY_STOP and raw_dist <= STOP_DIST)):
        ...
        cv2.putText(mask_bgr, f"Angle:{angle:.1f} Target:{target} Shift:{shift_px} Dist:{raw_dist:.0f}mm", (20, 70),
