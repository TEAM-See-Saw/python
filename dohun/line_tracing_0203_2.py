...
def setup_roi_exposure(cap):
    ...
    target_brightness = 125  # 우리가 원하는 ROI의 평균 밝기 (0~255 사이)
    
    for i in range(20): # 20프레임 동안 최적값 찾기
        ret, frame = cap.read()
        if not ret: break
...
def find_optimal_l_min(cap):
    ...
    try:
        ...
        detected_l = pixels[int(len(pixels) * 0.90)] # 상위 10% 지점의 밝기
...
def main():
    ...
    try:
        while True:
            ...
            # 4. 통신
            curr = time.time()
            if ser and curr - last_serial_time > SERIAL_DELAY:
                ser.write(f"S,{servo_val}\n".encode())
                last_serial_time = curr
            if ser and curr - last_speed_time > SPEED_REFRESH_DELAY:
