...
# HLS 필터 기준값
current_l_min = 160  # 밝기 시작값 (적당함)
MIN_L_VAL = 80       # 너무 어두워지지 않도록 하한선
MAX_L_VAL = 220      # 상한선
S_MAX_VAL = 80       # 채도 제한 (약간 누런 차선 허용)

# 노이즈 제거 필터값
MORPH_SIZE = (3, 3)  # 모폴로지 커널 크기 (작은 노이즈 제거)
BLUR_K = 5           # 미디언 블러 강도 (적당한 흐림 효과)
...
def main():
    ...
    cap.set(3, width)
    cap.set(4, height)
    #cap.set(15, -6) # 카메라 노출값 (환경에 따라 조절)
    ...
    try:
        while True:
            ...
            cv2.imshow("Indoor Mode", combined)
