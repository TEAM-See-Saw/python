...
# --- 통신 설정 ---
SERIAL_DELAY = 0.05
SPEED_REFRESH_DELAY = 0.5 

# --- 속도 & 모터 설정 ---
SPEED_NORMAL = 100
SPEED_SLOW = 60       # ★ [수정] 감속 속도를 100 -> 60으로 낮춤 (정확한 정지를 위해)
SPEED_STOP = 0
SERVO_CENTER = 570
SERVO_LEFT_MAX = 680
SERVO_RIGHT_MAX = 480
...
SHIFT_GAIN = 1.5
OBSTACLE_CLEAR_TIME = 0.5 # ★ [수정] 복귀 시간 단축 (1.5 -> 0.5) 차선 이탈 방지
...
# ★ [수정] 정지선 인식 범위 확대 (roi_ratio 0.6 -> 0.5)
def detect_stop_line(mask, frame_to_draw, roi_ratio=0.5): 
    h, w = mask.shape[:2]
...
try:
    ...
    cap_traffic.set(cv2.CAP_PROP_EXPOSURE, traffic_exp_value)
    ...
    cap_lane.set(cv2.CAP_PROP_EXPOSURE, lane_exp_value)
...
try:
    ...
    for scan in lidar.iter_scans():
        ...
        if not ret_t or not ret_l: print("❌ 카메라 끊김"); break
        ...
        roi_mask_poly = np.zeros_like(mask); cv2.fillPoly(roi_mask_poly, [lane_roi_points], 255)
        ...
        if raw_dist < OBSTACLE_START_DIST:
            ...
            if obstacle_count == 1:
                avoid_direction = 0; status_msg = "IGNORE (#1)"; status_color = (0, 255, 0)
            elif obstacle_count == 2:
                avoid_direction = -1; status_msg = "AVOID L (#2)"; status_color = (0, 255, 255)
            elif obstacle_count == 3:
                avoid_direction = 1; status_msg = "AVOID R (#3)"; status_color = (255, 0, 255)
            else:
                final_speed = SPEED_SLOW
        else:
            if is_obstacle_detected:
                # ★ [핵심 수정] 복귀 중일 때 강제로 회피하지 않고, 즉시 차선 중앙 유지를 시도 (avoid_direction = 0)
                # 단, 안정성을 위해 SPEED_SLOW 유지
        ...
        # (B) 신호등 및 감속 로직 (수정됨)
        # ==========================================
        # ★ 빨간불이면 무조건 미리 감속 (가장 중요)
        if traffic_light == "RED":
            final_speed = SPEED_SLOW
            status_msg = "RED LIGHT (SLOW)"
        ...
        if is_crosswalk_stop:
            ...
            if traffic_light == "GREEN":
                ...
                is_crosswalk_stop = False; crosswalk_cooldown_timer = current_time
                print(f"⚠️ Timeout GO!")
            else:
                status_msg = f"WAITING ({elapsed:.1f}s)"
            ...
            if has_stop_line and traffic_light == "RED":
                if crosswalk_detect_timer == 0:
                    ...
                    final_speed = SPEED_STOP
                    status_msg = "!!! STOP !!!"; status_color = (0, 0, 255)
                    print("🛑 STOP EXECUTE")
            else:
                crosswalk_detect_timer = 0
        ...
        # 회피 중일 때만 장애물 거리 반영, 복귀 중(0)일 때는 라인만 보고 주행
        eff_dist = raw_dist
        ...
        if traffic_light == "RED": cv2.circle(frame_traffic, (50, 100), 20, (0, 0, 255), -1)
